...
from sklearn.ensemble import RandomForestClassifier # для сравнения
...
model_RF = RandomForestClassifier(n_estimators=20, max_depth=6)
model_RF.fit(df_train, df_train_main)
predict_RF = model_RF.predict(df_test)

model_MRF = MyRandomForest(n_estimators=20, max_depth=6)
...
print('\nТочность RandomForest:',
      get_accuracy(predict_RF, df_test_main['label']), '%')
