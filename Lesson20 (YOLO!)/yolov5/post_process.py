...
def post_process(input_data, anchors):
    ...
    boxes, classes, scores = filter_boxes(boxes, scores, classes_conf) # filter according to threshold

    nboxes, nclasses, nscores = [], [], [] # nms
...
def draw(image, boxes, scores, classes):
    for box, score, cl in zip(boxes, scores, classes):
        # хз почему filter не убирает bbox-ы с низким score, ну пусть тут тоже будет
        if score >= OBJ_THRESH * 100:
...
if __name__ == '__main__':
    ...
    for i in range(len(img_list)):
        ...
        for name in output_names:
            try:
                output = torch.load(os.path.join(working_dir, name))
        ...
        output_dir = os.path.join(working_dir, '__postprocess')
